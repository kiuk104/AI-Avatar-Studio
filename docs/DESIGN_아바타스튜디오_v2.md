...
### 4-6. Photoshop 수작업 편집 — PSD 내보내기 (단계 7)
AI 생성은 비결정적이라, "거의 맞는데 한 칸만" 같은 경우 손으로 고치는 게 빠르다. 칸별 재생성(§4-4-2)의
보완으로, **레이어드 PSD 내보내기**를 두 곳에 둔다. **재임포트는 만들지 않는다** — 사용자가 PS에서 고친 뒤
PNG로 export해 **파일을 직접 교체**한다(앱 왕복 없음).

- **기능 B 감정 시트 → 레이어드 PSD**: 칸마다 **레이어 1개**(name = 표정 id, `left=(i%cols)*cell`, `top=floor(i/cols)*cell`,
  canvas = cell×cell), 캔버스 = 시트 전체 크기. **칸 경계 가이드** 포함(세로 `col*cell`, 가로 `row*cell`).
  Photoshop에서 표정별 레이어를 토글·정교 수정 → 평탄화 export → famillie-kim 에셋 PNG를 직접 교체.
- **기능 A 원본 아바타 → PSD**: 현재 아바타를 단일 레이어(name `avatar`, 투명 배경) PSD로. PS에서 디테일 보정 후
  PNG로 저장해 재사용(새 베이스로 업로드하거나 보관).
- **구현**: `ag-psd`의 `writePsd`로 브라우저에서 PSD(ArrayBuffer) 생성 → Blob 다운로드. 의존성 `ag-psd` 1개 추가.
  레이어 canvas는 기존 셀 dataURL(기능 B의 `cells` 상태)·아바타 dataURL을 그려 만든다. 가이드는 ag-psd guides 지원 사용
  (미지원 빌드면 반투명 그리드 레이어로 대체).
- 내보낸 PSD의 그리드/셀 크기는 manifest와 동일해야 교체 후 슬라이싱이 맞는다(평탄화 시 캔버스 크기 유지).
...
7. **PSD 내보내기**(§4-6): 기능 B 레이어드 PSD(칸=레이어+가이드) + 기능 A 아바타 PSD. `ag-psd` 추가. 재임포트 없음(파일 교체).

> 1~3이 "일관성 편집" 본질. 4~6이 "UX 재편". 1~6 구현 완료. 7은 수작업 편집 보강.
